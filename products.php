              ...
              if(!isset($_GET["category"]) && !isset($_GET["search_product"])){
                ...
                if (mysqli_num_rows($result_categories) > 0) {
                    while ($category = mysqli_fetch_assoc($result_categories)) {
                        ...
                        $sql_select_products = "SELECT * FROM `luxedrive_products` WHERE category_id=$category_id && status='available';";
                        ...
                        if (mysqli_num_rows($result_products) > 0) {
                          ...
                          while ($product = mysqli_fetch_assoc($result_products)) {
                              ...
                              $product_per_day = $product['product_per_day'];
                              $product_excess_mileage = $product['product_excess_mileage'];
                              ...
                              <a class='card__img_link' href='#'>
                                  <img class='card__img' src='images/$product_image' alt=$product_name />
                              </a>
                              <div class='card__text'>
                                  ...
                                  <div class='price'> 
                                        <div class='perday'>PER DAY(80KM): Rs. $product_per_day</div>
                                        <div class='excessmileage'>EXCESS MILEAGE: Rs. $product_excess_mileage</div>
                                  </div>
                                  <ul class='card__tagbox'>
                                      ...
                                      <li class='tag__item play blue'>
                                          <label for='submit$product_id' class='sbm_btn'>Rent Now</label>
                                      </li>
                                  </ul>
                              </div>
                          </article>";
                ...
                $sql_select_categories = "SELECT * FROM `luxedrive_categories` WHERE category_id=$category_id;";
                $sql_select_products = "SELECT * FROM `luxedrive_products` WHERE category_id=$category_id && status='available';";
                $result_products = mysqli_query($conn, $sql_select_products);
                $result_categories = mysqli_query($conn, $sql_select_categories);
                ...
                $sql_select_products = "SELECT * FROM `luxedrive_products` WHERE product_keywords like '%$search_product%' && status='available';";
                $result_products = mysqli_query($conn, $sql_select_products);
