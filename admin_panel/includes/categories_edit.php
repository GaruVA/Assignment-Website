<?php
    ...
    if(isset($_POST["update"])){
        ...
        $sql_update_category = "UPDATE `luxedrive_categories` SET category_name='$new_category_name' WHERE category_id=$category_id;";
        $result_update_category = mysqli_query($conn, $sql_update_category);
        if ($result_update_category) {
            echo "<script>alert('Success: Category \"$category_name\" has been updated.'); window.location.href = 'categories.php?view';</script>";
        } else {
