<?php
    ...
    if(isset($_POST['book']) && isset($_POST['to']) && isset($_POST['from'])) {
      ...
      $user_address_street = $row_select_user["user_address_street"];
      ...
      $total_price = $product_per_day * ($date_difference + 1) * 115/100;
      
      $sql_insert_order = "INSERT INTO `luxedrive_orders` (user_id,product_id,order_invoice,order_status,order_from,order_to,order_duration,order_amount,order_paid,order_payment,order_date,order_address_street,order_address_state,order_address_city,order_confirm) VALUES ($user_id,$product_id,$order_invoice,'$order_status','$validated_from_date','$validated_to_date',$date_difference + 1,$total_price,0,'$order_payment',NOW(),'$user_address_street','$user_address_state','$user_address_city','pending')";
  ...
  <section class="home">
      <div class="container">
        <div class="row">
          <div class="col-lg-8">
          <?php
              $sql_select_order = "SELECT * FROM `luxedrive_orders` WHERE user_id=$user_id && order_confirm='pending';";
            ...
            <form action="orders.php" method="post">
                  <div class="mb-3">
                      ...
                      <input type="text" id="order_address_street" name="order_address_street" value="<?php echo $order_address_street;?>" class="form-control" required>
                  </div>

                  <div class="row mb-3">
                      <div class="col-md-6">
                          <label for="order_address_state" class="form-label">State:</label>
                          <input type="text" id="order_address_state" name="order_address_state" value="<?php echo $order_address_state;?>" class="form-control" required>
                      </div>

                      <div class="col-md-6">
                          <label for="order_address_city" class="form-label">City:</label>
                          <input type="text" id="order_address_city" name="order_address_city" value="<?php echo $order_address_city;?>" class="form-control" required>
                      </div>
                  </div>
...
if(isset($_POST['order_from']) && isset($_POST['order_to'])){
  ...
  $sql_update_order = "UPDATE `luxedrive_orders` SET order_from='$validated_order_from',order_to='$validated_order_to',order_duration=$date_difference + 1,order_amount=$total_price WHERE user_id='$user_id' AND order_confirm='pending';";
